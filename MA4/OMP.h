...
// reduction max_stl
#pragma omp parallel for reduction(max:max_stl){
    for(int i = 0; i < data_size; ++i){
         if(data[i] > max_stl){
        max_stl = data[i];
        }
...
//reduction max_omp
#pragma omp parallel for reduction(max:max_omp) num_threads(num_threads){
    for(int i = 0; i < data_size; ++i){
        if(data[i] > max_omp){
            max_omp = data[i];
        }
...
cout << "STL Max: " << max_stl << endl;
cout << "OMP Max: " << max_omp << endl;
